...
public class ChangelogHelper implements org.citydb.database.util.ChangelogHelper {
    ...
    public boolean isChangelogEnabled(String schemaName, Connection connection) throws SQLException {
        // TODO semantic-review: ported from former SchemaAdapter.getChangelogEnabled(), which used
        // PostgreSQL "select exists(...)" syntax. Rewritten to Oracle-style "rownum" + rs.next().
        // Oracle stores all_triggers.owner upper-cased, so schema-name casing should be verified.
        String sql = "select 1 from all_triggers " +
                "where owner = ? " +
                "and table_name = '" + org.citydb.database.schema.Table.FEATURE.getName() + "' " +
